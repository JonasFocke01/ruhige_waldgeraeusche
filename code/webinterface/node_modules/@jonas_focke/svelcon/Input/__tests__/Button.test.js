...
describe('Button', () => {
    ...
    it('has the right bg-color', async () => {
        ...
        expect(getByText('Testing').closest('.block')?.getAttribute('class')).toContain('bg-primary');
    });
    it('has the right additionalClasses', async () => {
        const { getByText } = render(Button, {
            props: { text: 'Testing', additionalClasses: 'mt-10' }
        });
        expect(getByText('Testing').closest('.block')?.getAttribute('class')).toContain('mt-10');
    });
});
