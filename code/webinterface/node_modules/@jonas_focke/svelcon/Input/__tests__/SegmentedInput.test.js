...
describe('Segmented Input', () => {
    it('implements input logic', async () => {
        const { getByText, getAllByPlaceholderText, component } = render(SegmentedInput, {
            segmentCount: 4
        });
    ...
    it('fires Change Event', async () => {
        ...
        await waitFor(() => {
            expect(getAllByPlaceholderText('*')[0].closest('.w-8')?.getAttribute('class')).toContain('focus:border-surface');
        });
    });
});
